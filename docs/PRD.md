...
### 4.1 인증 · 권한 (ADR-0005)
- FR-A1: uid(SID) + secret 입력 로그인. 서버가 ClassIn API 헬스체크 호출로 자격 검증 후 세션 발급 (원장 진입로).
- FR-A2: secret은 서버 측 세션에만 보관(암호화 쿠키 또는 서버 세션), 브라우저 노출 금지.
- FR-A3: (옵션) 서버 `.env`에 고정 자격을 두고 접속 비밀번호만으로 로그인하는 "보안 연결" 모드 — 계정이 없을 때의 **원장 부트스트랩**.
- FR-A4: **로컬 계정**(`users`: username / PBKDF2 password_hash / role / branch_id / teacher_uid / active)으로
  로그인. 역할은 계정에 저장된 값이며 사용자가 고를 수 없다. ClassIn 자격은 서버(.env) 또는 관별 오버라이드에서 온다.
- FR-A5: 로그인 실패는 아이디/비밀번호를 구분하지 않는 공통 메시지 + IP당 5회 실패 시 60초 잠금.

역할 표:

| 역할 | 보이는 범위 | 생성/알림 | 관리(/admin) |
|---|---|---|---|
| `owner` (원장/대표) | 전체. 상단 관 전환으로 특정 관만 보기 | 제한 없음 | 관 CRUD, 코스·선생님 배정, 모든 계정 |
| `manager` (관장) | 자기 관의 코스·선생님·학생·기록 | 자기 관 선생님 범위 | 자기 관 선생님 계정 생성/비활성/비번 재설정 |
| `teacher` (선생님) | 자기가 가르친 기록과 담당 코스 | 본인 uid로만 생성, 자기 학생만 알림 | 없음 (403) |

- 미배정 코스(`courses.branch_id IS NULL`)는 owner에게만 보인다.
- 스코프는 라우터가 아니라 store 계층에서 강제된다 — 페이지용 조회 함수는 `scope`를 **필수 키워드 인자**로 받는다.
