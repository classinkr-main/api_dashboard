...
| # | 항목 | 우선순위 | 비고 |
|---|---|---|---|
| 3-1 | **학부모 연락처 입력/관리 화면** | 높음 | 현재 `students.parent_phone`을 채울 UI가 없음 → 알림 화면이 "연락처 없음"만 표시. 학생 목록에서 인라인 편집 or CSV 업로드 (`/admin`에 붙이면 자연스러움) |
| 3-2 | **학생/선생님 수동 등록 화면** | 높음 | *부분 반영(ADR-0005)*: `/admin`에서 선생님→관 배정과 선생님 연결 계정 생성이 가능해졌다. 남은 것 — 학생/선생님 신규 **등록**(uid·이름 입력) 화면 |
| 3-3 | 토론-리마인드 우회 기능 | 중간 | `createActivityNoClass(type=6)` + release. **실계정에서 앱 푸시 여부 1회 검증 후** `/notify`에 버튼 추가 (기본 비활성) |
...
| 3-10 | 멀티 테넌트(학원 여러 곳) | v2 | *부분 반영(ADR-0005)*: 관별 SID/secret 오버라이드로 별도 ClassIn 계정 관 운영 가능. 남은 것 — 테넌트 단위 데이터 분리·과금 |

## 4. 기술 부채 / 보완

- **credential 모드 세션이 서버 재시작 시 소멸** (의도된 결정, ADR-0002) — 파일럿은 fixed 모드 권장. 필요 시 암호화 저장 검토
- 로그인 시도 rate limit: 프로세스 메모리 기반 IP당 5회/60초 잠금만 있음 (ADR-0005) — 다중 워커/재시작에는 무력하므로 nginx `limit_req` 병행 권장
