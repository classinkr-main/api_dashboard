...
- **생성(원격 실행)** — 선생님용: 스케줄을 붙여넣으면(표·자유 텍스트 무관) 자동으로 파싱하고
  등록된 선생님·기존 코스에 배정해 코스/수업을 일괄 생성, 미제출 학생 알림 문구 생성
...
| 경로 | 설명 |
|---|---|
| `/login` | 계정(아이디+비밀번호) 로그인. 계정이 없을 땐 ClassIn SID/secret 또는 `fixed` 모드 접속 비밀번호로 원장 진입 |
...
| 역할 | 보이는 범위 | 생성/알림 | `/admin` |
|---|---|---|---|
| `owner` 원장/대표 | 전체 (상단에서 관 전환 가능) | 제한 없음 | 전체 |
| `manager` 관장 | 자기 관의 코스·선생님·학생·기록 | 자기 관 선생님 범위 | 자기 관 선생님 계정만 |
| `teacher` 선생님 | 본인이 가르친 기록과 담당 코스 | 본인 uid로만 생성 | 접근 불가 (403) |

첫 진입은 `fixed` 모드 접속 비밀번호(또는 SID/secret) → 원장 세션이며, 이후 `/admin`에서 관과 계정을
만들면 각자 아이디/비밀번호로 로그인한다. 미배정 코스(관이 없는 코스)는 원장에게만 보인다.
